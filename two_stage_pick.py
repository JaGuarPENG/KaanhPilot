...
class SnapshotPickApp:
    ...
    def __init__(self, config_path: Path = DEFAULT_CONFIG_PATH) -> None:
        ...
        self._monitor_robot = KaanhRobotBackend(
            ...
            self._config.robot.monitor_port,
        ...
        self._single_shot_left_hand = SnapShotCommand(
            robot=self._control_robot,
            camera=self._camera_left_hand,
            detector=self._detector_drink,
        ...
        self._target_ids = self._detector_drink.target_ids
        ...
        self._two_stage_pick_workflow = TwoStagePickWorkflow(
            robot=self._control_robot,
            robot_executor=RobotCommandExecutor(self._control_robot),
            hand_executor=HandCommandExecutor(self._control_robot),
            snapshot_command=self._single_shot_left_hand,
        )
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self._single_shot_left_hand.initialize_resources()
    ...
    def run(self) -> None:
        ...
        print("\nH: Home | P: 单帧拍照 | 1/2/3/4: 选择并抓取 | Esc: 取消 | Q: 退出")
    ...
    def _connect_control_robot(self) -> None:
        ...
        self._control_robot.set_pgm_vel(70)
        self._control_robot.set_jog_vel(70)
        ...
        while not self._stop_event.is_set():
            ...
            try:
                if command == "move_home":
                    ...
                    #此处执行抓取
                    self._two_stage_pick_workflow.execute(model_id=0, target_id=payload)
                else:
        ...
        while not self._stop_event.is_set():
            ...
            try:
                if command != "capture_once":
                    continue
                observation = self._camera_left_hand.get_latest_observation()
                candidates: dict[str, SnapshotCandidate] = {}
                for index, target_id in enumerate(self._target_ids, start=1):
                    detections = self._detector_drink.detect(observation, target_id).detections
                    if not detections:
                        continue
    ...
    def _draw_snapshot(
        ...
        colors = {"1": (0, 220, 0), "2": (0, 210, 255), "3": (255, 120, 0),"4": (255, 0, 255)}
        ...
        cv2.putText(
            image,
            "1/2/3/4 select | P recapture | Esc cancel | Q quit",
            (12, 28),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.55,
            (255, 255, 255),
